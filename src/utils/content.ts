 ...
 */
async function _getPosts(lang?: string, hide = true) {
  ...
  const filteredPosts = await getCollection(
    ...
    ({ data }: CollectionEntry<'posts'>) => {
      ...
      const shouldInclude = !(hide && data.hidden) && (import.meta.env.DEV || !data.draft)
...
async function _getPostsGroupByTags(lang?: string) {
  const posts = await getPosts(lang, false)
